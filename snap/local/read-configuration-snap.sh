...
snapctl set remote-server-port="$(yq '(.remote-server-port // (.remote-server-port = 2222))' $CONFIGURATION_FILE_PATH)"
snapctl set storage-base-path="$(yq '(.storage-base-path // (.storage-base-path = "/var/lib/caddy-fileserver/"))' $CONFIGURATION_FILE_PATH)"
snapctl set max-bag-duration="$(yq '(.max-bag-duration // (.max-bag-duration = 300))' $CONFIGURATION_FILE_PATH)"
snapctl set max-bag-size="$(yq '(.max-bag-size // (.max-bag-size = 250000000))' $CONFIGURATION_FILE_PATH)"
