...
LIST_OF_FILES=$(fdfind --type file --change-older-than 15min . $SNAP_COMMON/) # TODO make the time configurable

# remove old files
rm -f $LIST_OF_FILES

# get the list of empty directories
LIST_OF_EMPTY_DIRS=$(fdfind --type directory --type empty . $SNAP_COMMON/)
